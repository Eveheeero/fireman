...
pub trait FireRaw: Fire {
    ...
    /// Returns a deterministic map of predefined symbol names by virtual address.
    ///
    /// When multiple names exist for the same address, the lexicographically smallest
    /// name is selected to keep output deterministic.
    fn get_pre_defined_symbol_map(&self) -> HashMap<u64, String> {
        let defined = self.get_defined();
        let reader = defined.get_reader();
