        ...
        match chars[i] {
            ...
            c if c.is_ascii_digit() => {
                ...
                if let Ok(n) = num_str.parse::<i64>() {
                    ...
                    // Large numbers that overflow i64 are stored as identifiers;
                    // UInt matching parses them as u64.
...
// ---------------------------------------------------------------------------
// Expression-level annotation: match & set .comment on matched expressions
// ---------------------------------------------------------------------------

/// Walk all expressions in `stmts` bottom-up. For each expression, try to match
/// `match_pat` with `predicates`. If matched, set or append the comment text
/// on the expression. Returns `true` if any annotation was added.
pub fn annotate_expressions_in_stmts(
    stmts: &mut [WrappedAstStatement],
    match_pat: &PatTree,
    predicates: &[WherePredicate],
    comment: &str,
    ...
    let mut changed = false;
    for stmt in stmts.iter_mut() {
        changed |= annotate_expressions_in_statement(stmt, match_pat, predicates, comment);
    }
    changed
}

fn annotate_expressions_in_statement(
    stmt: &mut WrappedAstStatement,
    match_pat: &PatTree,
    predicates: &[WherePredicate],
    comment: &str,
    ...
    match &mut stmt.statement {
        AstStatement::Declaration(_lhs, rhs) => {
            if let Some(rhs) = rhs {
                annotate_expression(rhs, match_pat, predicates, comment)
            } else {
                false
            }
        }
        AstStatement::Assignment(lhs, rhs) => {
            let a = annotate_expression(lhs, match_pat, predicates, comment);
            let b = annotate_expression(rhs, match_pat, predicates, comment);
            a || b
        }
        AstStatement::If(cond, branch_true, branch_false) => {
            let a = annotate_expression(cond, match_pat, predicates, comment);
            let b = annotate_expressions_in_stmts(branch_true, match_pat, predicates, comment);
            let c = if let Some(bf) = branch_false {
                annotate_expressions_in_stmts(bf, match_pat, predicates, comment)
            } else {
                false
            };
            a || b || c
        ...
        AstStatement::While(cond, body) | AstStatement::DoWhile(cond, body) => {
            let a = annotate_expression(cond, match_pat, predicates, comment);
            let b = annotate_expressions_in_stmts(body, match_pat, predicates, comment);
            a || b
        }
        AstStatement::For(init, cond, update, body) => {
            let a = annotate_expressions_in_statement(init, match_pat, predicates, comment);
            let b = annotate_expression(cond, match_pat, predicates, comment);
            let c = annotate_expressions_in_statement(update, match_pat, predicates, comment);
            let d = annotate_expressions_in_stmts(body, match_pat, predicates, comment);
            a || b || c || d
        }
        AstStatement::Switch(discrim, cases, default) => {
            let mut changed = annotate_expression(discrim, match_pat, predicates, comment);
            for (_lit, case_body) in cases.iter_mut() {
                changed |=
                    annotate_expressions_in_stmts(case_body, match_pat, predicates, comment);
            }
            if let Some(default_body) = default {
                changed |=
                    annotate_expressions_in_stmts(default_body, match_pat, predicates, comment);
            }
            changed
        }
        AstStatement::Block(body) => {
            annotate_expressions_in_stmts(body, match_pat, predicates, comment)
        }
        AstStatement::Return(expr) => {
            if let Some(expr) = expr {
                annotate_expression(expr, match_pat, predicates, comment)
            } else {
                false
            }
        }
        AstStatement::Call(call) => annotate_call(call, match_pat, predicates, comment),
        _ => false,
    }
}
...
fn annotate_expression(
    ...
    // Recurse into children first (bottom-up).
    let mut changed = match &mut expr.item {
        AstExpression::UnaryOp(_, arg) => annotate_expression(arg, match_pat, predicates, comment),
        AstExpression::BinaryOp(_, left, right) => {
            let a = annotate_expression(left, match_pat, predicates, comment);
            let b = annotate_expression(right, match_pat, predicates, comment);
            a || b
        }
        AstExpression::Call(call) => annotate_call(call, match_pat, predicates, comment),
        AstExpression::Cast(_, arg)
        | AstExpression::Deref(arg)
        | AstExpression::AddressOf(arg)
        | AstExpression::MemberAccess(arg, _) => {
            annotate_expression(arg, match_pat, predicates, comment)
        }
        AstExpression::ArrayAccess(base, idx) => {
            let a = annotate_expression(base, match_pat, predicates, comment);
            let b = annotate_expression(idx, match_pat, predicates, comment);
            a || b
        }
        AstExpression::Ternary(cond, true_expr, false_expr) => {
            let a = annotate_expression(cond, match_pat, predicates, comment);
            let b = annotate_expression(true_expr, match_pat, predicates, comment);
            let c = annotate_expression(false_expr, match_pat, predicates, comment);
            a || b || c
        }
        AstExpression::Literal(_)
        | AstExpression::Variable(_, _)
    ...
    // Try to match the current expression and annotate it.
...
fn annotate_call(
    ...
    match call {
        ...
        | AstCall::Function { args, .. } => {
            let mut changed = false;
            for arg in args.iter_mut() {
                changed |= annotate_expression(arg, match_pat, predicates, comment);
            }
            changed
        }
        AstCall::Builtin(_, _) => false,
    }
}
