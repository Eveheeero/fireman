//! Operator normalization and comparison simplification.
//!
//! Canonicalizes expressions for consistent form:
//!   Not(Not(x)) → x, Not(cmp(a,b)) → inv_cmp(a,b),
//!   literal op x → x op literal (commutative), if(!c){A}else{B} → if(c){B}else{A}
...
pub(crate) fn canonicalize_operators(
    ast: &mut Ast,
...
pub(crate) fn canonicalize_condition_expression(expr: &mut Wrapped<AstExpression>) {
...
/// `x != 0` → `x`, `x == 0` → `!x`
pub(crate) fn simplify_condition_zero_cmp(expr: &mut Wrapped<AstExpression>) {
