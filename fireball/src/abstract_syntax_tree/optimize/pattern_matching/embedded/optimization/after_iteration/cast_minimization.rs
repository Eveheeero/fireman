//! Redundant cast removal extracted from the original optimization pipeline.
//!
//! Simplifies nested and identity casts:
//!   Cast(T1, Cast(T2, x))           →  Cast(T1, x)
//!   CastSigned(CastUnsigned(x))     →  CastSigned(x)
//!   Cast(T, Literal(n)) where fits  →  Literal(n)

use crate::{
    abstract_syntax_tree::{
        Ast, AstBuiltinFunctionArgument, AstCall, AstExpression, AstFunctionId, AstFunctionVersion,
        AstLiteral, AstStatement, AstUnaryOperator, AstValueType, AstOptimizationKind, Wrapped,
        WrappedAstStatement,
    },
    prelude::DecompileError,
};

pub(crate) fn minimize_casts(
    ast: &mut Ast,
