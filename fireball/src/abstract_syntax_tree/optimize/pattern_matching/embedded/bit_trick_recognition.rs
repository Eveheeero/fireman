//! Rotation recovery, strength reduction reversal, and bit-trick annotation
...
/// Apply the original Rust annotation heuristics for the bit-trick patterns
/// that now have `.fb` equivalents. This remains the fallback when
/// `use_embedded_passes` is enabled.
pub(crate) fn annotate_translated_patterns(
    ast: &mut Ast,
    function_id: AstFunctionId,
    function_version: AstFunctionVersion,
) -> Result<(), DecompileError> {
    ...
    annotate_patterns_in_statement_list(&mut body);
...
fn annotate_patterns_in_statement_list(stmts: &mut Vec<WrappedAstStatement>) {
    for stmt in stmts.iter_mut() {
        annotate_patterns_in_statement(stmt);
    }
}
...
fn annotate_patterns_in_statement(stmt: &mut WrappedAstStatement) {
    match &mut stmt.statement {
        AstStatement::Declaration(_lhs, rhs) => {
            if let Some(rhs) = rhs {
                annotate_patterns_in_expression(rhs);
            }
        }
        AstStatement::Assignment(lhs, rhs) => {
            annotate_patterns_in_expression(lhs);
            annotate_patterns_in_expression(rhs);
        }
        AstStatement::If(cond, branch_true, branch_false) => {
            annotate_patterns_in_expression(cond);
            annotate_patterns_in_statement_list(branch_true);
            if let Some(branch_false) = branch_false {
                annotate_patterns_in_statement_list(branch_false);
            }
        }
        AstStatement::While(cond, body) | AstStatement::DoWhile(cond, body) => {
            annotate_patterns_in_expression(cond);
            annotate_patterns_in_statement_list(body);
        }
        AstStatement::For(init, cond, update, body) => {
            annotate_patterns_in_statement(init);
            annotate_patterns_in_expression(cond);
            annotate_patterns_in_statement(update);
            annotate_patterns_in_statement_list(body);
        }
        AstStatement::Switch(discrim, cases, default) => {
            annotate_patterns_in_expression(discrim);
            for (_lit, case_body) in cases.iter_mut() {
                annotate_patterns_in_statement_list(case_body);
            }
            if let Some(default_body) = default {
                annotate_patterns_in_statement_list(default_body);
            }
        }
        AstStatement::Block(body) => {
            annotate_patterns_in_statement_list(body);
        ...
        AstStatement::Return(expr) => {
            if let Some(expr) = expr {
                annotate_patterns_in_expression(expr);
            }
        }
        AstStatement::Call(call) => {
            annotate_patterns_in_call(call);
        }
        AstStatement::Goto(_)
        | AstStatement::Assembly(_)
        | AstStatement::Ir(_)
...
fn annotate_patterns_in_call(call: &mut AstCall) {
    match call {
        AstCall::Variable { args, .. }
        | AstCall::Function { args, .. }
        | AstCall::Unknown(_, args) => {
            for arg in args.iter_mut() {
                annotate_patterns_in_expression(arg);
            }
        }
        AstCall::Builtin(_, _) => {}
    }
...
fn annotate_patterns_in_expression(expr: &mut Wrapped<AstExpression>) {
    match &mut expr.item {
        AstExpression::UnaryOp(_, arg) => {
            annotate_patterns_in_expression(arg);
        }
        AstExpression::BinaryOp(_, left, right) => {
            annotate_patterns_in_expression(left);
            annotate_patterns_in_expression(right);
        }
        AstExpression::Call(call) => {
            annotate_patterns_in_call(call);
        }
        ...
        AstExpression::ArrayAccess(base, idx) => {
            annotate_patterns_in_expression(base);
            annotate_patterns_in_expression(idx);
        }
        AstExpression::Ternary(cond, true_expr, false_expr) => {
            annotate_patterns_in_expression(cond);
            annotate_patterns_in_expression(true_expr);
            annotate_patterns_in_expression(false_expr);
        }
        AstExpression::Variable(_, _)
        | AstExpression::Unknown
        | AstExpression::Undefined
    ...
    if expr.comment.is_none() {
        if let Some(comment) = try_recognize_branchless_abs(&expr.item) {
            expr.comment = Some(comment);
            return;
        }
    }
    ...
    if expr.comment.is_none() {
        if let Some(comment) = try_recognize_bswap16(&expr.item) {
            expr.comment = Some(comment);
        }
    }
}
...
fn try_recognize_branchless_abs(expr: &AstExpression) -> Option<String> {
    use crate::abstract_syntax_tree::optimize::opt_utils::expr_structurally_equal;

    let AstExpression::BinaryOp(AstBinaryOperator::Sub, lhs, rhs) = expr else {
        return None;
    };
    let AstExpression::BinaryOp(AstBinaryOperator::RightShift, base_r, shift_r) = &rhs.item else {
        return None;
    };
    ...
    let AstExpression::BinaryOp(AstBinaryOperator::BitXor, base_l, xor_rhs) = &lhs.item else {
        return None;
    };
    let AstExpression::BinaryOp(AstBinaryOperator::RightShift, base_xor, shift_xor) = &xor_rhs.item
    else {
        return None;
    };
    if extract_literal_u64(&shift_xor.item) != shift_val {
        return None;
    }
    if !expr_structurally_equal(&base_r.item, &base_l.item)
        || !expr_structurally_equal(&base_r.item, &base_xor.item)
    {
        return None;
    }
    Some("branchless abs (intrinsic idiom)".to_string())
}
...
fn try_recognize_overflow_check(expr: &Wrapped<AstExpression>) -> Option<String> {
    ...
    match cmp_op {
        ...
        AstBinaryOperator::NotEqual => {
            if matches!(
                lhs.item,
                AstExpression::BinaryOp(AstBinaryOperator::Div, _, _)
            ) || matches!(
                rhs.item,
                AstExpression::BinaryOp(AstBinaryOperator::Div, _, _)
            ) {
...
fn try_recognize_saturating(expr: &Wrapped<AstExpression>) -> Option<String> {
    ...
    if matches!(
        ...
        if let Some(c) = extract_literal_u64(&rhs.item) {
            ...
            {
                return Some("saturating clamp (max)".to_string());
            }
        }
    }
    ...
    if matches!(op, AstBinaryOperator::Less | AstBinaryOperator::LessEqual) {
        if let Some(c) = extract_literal_u64(&rhs.item) {
            if extract_literal_u64(&true_expr.item) == Some(c)
                && expr_structurally_equal(&lhs.item, &false_expr.item)
            {
                return Some("saturating clamp (min)".to_string());
            }
        }
    }
...
fn try_recognize_intrinsic_idiom(expr: &AstExpression) -> Option<String> {
    if let AstExpression::BinaryOp(AstBinaryOperator::BitXor, _lhs, rhs) = expr {
        if let AstExpression::BinaryOp(AstBinaryOperator::BitAnd, and_lhs, _and_rhs) = &rhs.item {
            if matches!(
                and_lhs.item,
                AstExpression::BinaryOp(AstBinaryOperator::BitXor, _, _)
            ) {
...
fn try_recognize_bswap16(expr: &AstExpression) -> Option<String> {
