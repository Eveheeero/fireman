...
pub(crate) fn suppress_static_guards(
    ast: &mut Ast,
...

    {
        ...
        function
            ...
            .push(AstOptimizationKind::PatternMatching(Box::new(
                AstPattern::predefined_pattern("static-guard-suppression.fb").unwrap(),
            )));
    }
...
mod tests {
    ...
    fn matches_direct_function_calls_by_resolved_name() {
        let target = AstFunctionId { address: 0x403000 };
        let version = AstFunctionVersion(1);
        let function_versions = HashMap::from([(target, version)]);
        let functions = build_function_map(target, version, "__cxa_guard_acquire");
