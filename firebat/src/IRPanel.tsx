          ...
          {decompileResult?.data.ir && decompileResult.data.ir.length > 0 ? (
            <div className="mt-4 space-y-2">
              {decompileResult.data.ir.map((irItem, index) => (
                <pre key={index} className={`${decompileResult.colors.get(irItem.parentsAssemblyIndex) || 'bg-gray-100'} p-3 rounded border text-sm font-mono whitespace-pre-wrap overflow-auto text-left select-text`}>
                  {irItem.data}
                </pre>
              ))}
            </div>
