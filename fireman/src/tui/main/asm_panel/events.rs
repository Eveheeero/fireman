...
pub fn move_cursor(ctx_: &MutexCtx, vector: i32) -> std::io::Result<bool> {
    ...
    let list_len = ctx.main_context.asm_context.list.lock().unwrap().len();
    if result < 0 {
        ctx.main_context.asm_context.list_cursor = None;
    } else if list_len == 0 {
        ctx.main_context.asm_context.list_cursor = None;
    } else if result >= list_len as i32 {
        ctx.main_context.asm_context.list_cursor = Some(list_len - 1);
    } else {
        ctx.main_context.asm_context.list_cursor = Some(result as usize);
    }
